...
# ============================================================================
# BOT MARKET PRICE ENTRY - Tạo lệnh 1 theo giá thị trường
# Khác biệt với hd_order.py: Không đọc activation price từ sheet,
# mà dùng lastPrice (giá thị trường) làm activation price
# ============================================================================
...
# Tạo tên file log với timestamp: hd_order_market_price_dd_mm_yyyy_h_m_s.log
log_timestamp = datetime.now().strftime('%d_%m_%Y_%H_%M_%S')
log_filename = logs_dir / f'hd_order_market_price_{log_timestamp}.log'
...
order_log_path = logs_dir / 'order_market_price.log'
...
def do_it():
  print(f"{datetime.now()}. Scan Vào Lệnh (MARKET PRICE)----", flush=True)
  sys.stdout.flush()  # Flush ngay sau khi bắt đầu scan
  logger.info(f"{datetime.now()}. Scan Vào Lệnh (MARKET PRICE)----")
  ...
  if state_value == STATE_STOP:
    ...
    print(f"🔍 Scan {state_value} từ hàng {start_row} đến {end_row} (MARKET PRICE)", flush=True)
    logger.info(f"Scan {state_value} từ hàng {start_row} đến {end_row} (MARKET PRICE)")
    ...
    for d in don_bay:
        ...
        # ============================================================================
        # CẤU TRÚC SHEET: A=Symbol, B=Leverage, C=Callback, H=Capital
        # KHÔNG ĐỌC cột D (Activation Price) - Dùng giá thị trường
        # ============================================================================
        leverage_idx = 1    # Cột B
        callback_idx = 2    # Cột C
        # activation_idx = 3  # Cột D - KHÔNG SỬ DỤNG cho market price bot
        capital_idx = 7     # Cột H
        ...
        # ============================================================================
        # KHÔNG VALIDATE cột D (Activation Price) - Bot dùng giá thị trường
        # ============================================================================
        ...
        try:
            ...
            print(f"🎯 Vào lệnh 1 {state_value} (MARKET PRICE): {sym} (Leverage {d[leverage_idx]}x)", flush=True)
            logger.info(f"--- Vào lệnh 1 {state_value} (MARKET PRICE): {sym} TRAILING_STOP đòn bẩy: {d[leverage_idx]} [B2 confirmed: {current_state} @ {check_time.strftime('%H:%M:%S')}]")
            ...
            # ============================================================================
            # [MARKET PRICE ENTRY] Lấy giá thị trường hiện tại làm activation price
            # Thay vì đọc từ cột D, bot sẽ dùng lastPrice (giá thị trường)
            # ============================================================================
            ...
            # Dùng giá thị trường làm activation price
            activation_price_raw = lastPrice
            
            logger.info(f"[MARKET PRICE] {symbol} - Giá thị trường hiện tại: {activation_price_raw}")
            print(f"📊 {symbol} - Giá thị trường: {activation_price_raw}", flush=True)
            
            # Validate activation_price > 0
            if activation_price_raw <= 0:
                print(f"⚠️  {symbol}: Giá thị trường = {activation_price_raw} (phải > 0), bỏ qua", flush=True)
                logger.warning(f"{symbol}: Giá thị trường = {activation_price_raw} (phải > 0), bỏ qua")
                continue
            
            # Làm tròn activation price theo precision của Binance
            try:
                activation_price_str = exchange.price_to_precision(symbol, activation_price_raw)
                activation_price = float(activation_price_str)
                logger.info(f"[MARKET PRICE] {symbol} - Sau khi price_to_precision(): {activation_price}")
                print(f"📊 {symbol} - Giá sau khi làm tròn: {activation_price}", flush=True)
            except Exception as e:
                ...
                try:
                    ...
                    print(f"⚠️  {symbol}: Không thể làm tròn giá thị trường: {e2}, bỏ qua", flush=True)
                    logger.error(f"{symbol}: Không thể làm tròn giá thị trường: {e2}", exc_info=True)
                    continue
            
            # Validate activation_price > 0 sau khi làm tròn
            if activation_price <= 0:
                print(f"⚠️  {symbol}: Giá sau khi làm tròn = {activation_price} (phải > 0), bỏ qua", flush=True)
                logger.warning(f"{symbol}: Giá sau khi làm tròn = {activation_price} (phải > 0), bỏ qua")
                continue
            ...
            # ============================================================================
            # [MARKET PRICE] KHÔNG CẦN validate activation_price vs lastPrice
            # Vì activation_price = lastPrice, lệnh sẽ trigger ngay lập tức
            # Đây là hành vi mong muốn cho "market entry"
            # ============================================================================
            
            # Tính toán và làm tròn số lượng (Quantity)
            ...
            # Tính notional = activation_price * amount
            ...
            print(f"📤 Tạo Trailing Stop (MARKET PRICE): {symbol} {side} @ {activation_price}, callback={callback_rate}%", flush=True)
            logger.info(f"Tạo Trailing Stop (MARKET PRICE): {symbol} {side} @ {activation_price}, callback={callback_rate}%")
            ...
            msg = f"✅ <b>LỆNH CHỜ (MARKET PRICE TRAILING STOP)</b>\n\n<b>Mã:</b> {symbol}\n<b>Side:</b> {type}\n<b>Giá thị trường:</b> {activation_price}\n<b>Callback:</b> {callback_rate}%\n<b>Đòn bẩy:</b> {leverage}x\n<b>Vốn:</b> {capitalMoney} USDT"
            
            # Log vào order.log
            order_logger.info(f"LỆNH 1 (Market Price Entry) | {symbol} | {type} | Market Price: {activation_price} | Callback: {callback_rate}% | Leverage: {leverage}x | Capital: {capitalMoney} USDT | Order ID: {order.get('id', 'N/A')}")
    ...
    print(f"✅ Hoàn thành scan {state_value} (MARKET PRICE) - Đã xử lý {row_count}/{len(don_bay)} dòng", flush=True)
    logger.info(f"[SCAN END] Hoàn thành scan {state_value} (MARKET PRICE) - Đã xử lý {row_count}/{len(don_bay)} dòng")
...
print(f"🚀 Khởi động bot MARKET PRICE - Chạy mỗi {cst.delay_vao_lenh} giây", flush=True)
logger.info(f"Khởi động bot MARKET PRICE - Chạy mỗi {cst.delay_vao_lenh} giây")
